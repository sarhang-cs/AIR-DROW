...
# AIR-DROW v6.5.0 — Phase 5 safe source replacement helper for Termux.
...
for file in package.json package-lock.json web/index.html scripts/preflight.mjs scripts/verify-phase4-project-safety.mjs scripts/verify-phase6-device-readiness.mjs scripts/sync-official-hand-model.mjs scripts/build-selfhosted-mediapipe.mjs; do
...
(cd "$SOURCE_DIR" && node scripts/preflight.mjs && node scripts/verify-foundation-hardening.mjs && node scripts/verify-phase2-ui-system.mjs && node scripts/verify-phase3-hand-drawing.mjs && node scripts/verify-phase4-project-safety.mjs && node scripts/verify-phase6-device-readiness.mjs && node scripts/validate-inline.mjs)
mkdir -p "$BACKUP_DIR"
shopt -s dotglob nullglob
for item in "$TARGET_DIR"/*; do
  [[ "$(basename "$item")" == ".git" ]] && continue
