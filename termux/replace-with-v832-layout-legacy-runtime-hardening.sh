...
# Transactional replacement: preserves .git and the existing local Kurdish font.
set -Eeuo pipefail
...
[ -f "$SOURCE/docs/V832_LAYOUT_LEGACY_RUNTIME_HARDENING_KU.md" ] || { echo "The v8.3.2 guide is missing."; exit 1; }
mkdir -p "$BACKUP"
shopt -s dotglob nullglob
for item in "$REPO"/*; do [ "$(basename "$item")" = ".git" ] && continue; mv -- "$item" "$BACKUP/"; done
RESTORE_REQUIRED=1
cp -a "$SOURCE"/. "$REPO"/
# The source update intentionally does not overwrite the local Kurdish font asset.
# Preserve the known-good copy already present in the user’s repository.
if [ -d "$BACKUP/web/assets/fonts/noto-kufi-arabic" ]; then
  mkdir -p "$REPO/web/assets/fonts/noto-kufi-arabic"
  cp -a "$BACKUP/web/assets/fonts/noto-kufi-arabic"/. "$REPO/web/assets/fonts/noto-kufi-arabic"/
fi
...
RESTORE_REQUIRED=0
git add -A
if ! git diff --cached --quiet; then git commit -m "AIR-DROW v8.3.2 layout and legacy runtime hardening"; fi
