...
[ -f "$SOURCE/docs/V840_HAND_INPUT_RELIABILITY_KU.md" ] || { echo "The v8.4 hand reliability guide is missing."; exit 1; }
mkdir -p "$BACKUP"
shopt -s dotglob nullglob
for item in "$REPO"/*; do [ "$(basename "$item")" = ".git" ] && continue; mv -- "$item" "$BACKUP/"; done
RESTORE_REQUIRED=1
cp -a "$SOURCE"/. "$REPO"/
# Preserve the known-good local Kurdish font from the existing repository.
...
RESTORE_REQUIRED=0
git add -A
if ! git diff --cached --quiet; then git commit -m "AIR-DROW v8.4.0 hand input reliability"; fi
