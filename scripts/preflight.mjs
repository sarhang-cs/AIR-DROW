...
const required = [
  ...
  "web/vendor/models/hand_landmarker.task", "scripts/build-selfhosted-mediapipe.mjs", "scripts/verify-final-release.mjs", "scripts/verify-production-clean.mjs", "scripts/verify-local-hand-model.mjs", "scripts/verify-hand-runtime-loader-fix.mjs", "scripts/verify-deployment-build-order.mjs", "scripts/sync-toolbar-icons.mjs", "scripts/verify-toolbar-icon-bundle.mjs", "api/health.js", "api/ai/generate.js", ".env.example", "vercel.json", "package.json"
];
...
if (!manifest.name.includes("AIR-DROW") || !manifest.start_url.includes("v750")) throw new Error("Install manifest is missing the current release.");
if (!html.includes('production-ui.css?v=750')) throw new Error("Consolidated production stylesheet is not linked.");
for (const obsolete of ["phase2-ui.css", "ui-clarity.css", "final-layout-localization.css", "production-clean.css"]) if (html.includes(obsolete) || worker.includes(obsolete)) throw new Error(`Obsolete stylesheet reference remains: ${obsolete}`);
