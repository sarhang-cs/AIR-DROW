...
for (const file of ["web/assets/js/features/exporter.js", "web/assets/js/features/gesture-shortcuts.js", "web/assets/js/features/stroke-smoother.js", "web/assets/js/app.js", "web/assets/css/production-ui.css", "tests/export-gesture-reliability.test.mjs"]) {
  if (!existsSync(resolve(root, file))) throw new Error(`Missing input/export safety file: ${file}`);
}
...
for (const id of ["quickSaveBtn", "handPalette", "exportScale", "exportLayout", "exportQuality", "exportCameraComposite"]) if (!html.includes(`id=\"${id}\"`)) throw new Error(`Missing export/input UI control: ${id}`);
if (!html.includes('data-airdrow-surface="hand-palette"')) throw new Error("Hand palette must remain a protected UI surface.");
if (!runtime.includes("gestureShortcuts: false") || !runtime.includes("showHandGuide: true") || !runtime.includes("inputSafetyVersion: 5")) throw new Error("Safe hand defaults are missing.");
...
if (!(exporter.indexOf("const strokeRendererUsed") < exporter.indexOf("let snapshotUsed")) || !exporter.includes("if (!strokeRendererUsed &&")) throw new Error("Raster export must be source-first with a snapshot-only fallback.");
if (!app.includes("bindPhysicalAction") || !app.includes("event?.isTrusted")) throw new Error("File actions must be protected by a trusted physical-action gate.");
...
console.log("AIR-DROW input/export safety QA passed.");
