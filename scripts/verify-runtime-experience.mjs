...
for (const token of ["bindImmediateTextEntry", "primeHandAssetsForCamera", "scanWaitForEngine", "scanPlaceHandNow", "calibrationCheckPassed", "refreshDynamicLanguageSurfaces"]) {
  if (!app.includes(token) && !translations.includes(token)) throw new Error(`Missing runtime experience contract: ${token}`);
}
...
if (!css.includes("hand-calibration-overlay.is-hiding") || !css.includes("mobile text-entry contract")) throw new Error("Mobile input or calibration success transition styles are missing.");
console.log("AIR-DROW runtime experience verification passed.");
