...
const finalLayout = read("web/assets/css/final-layout-localization.css");
...
]) if (!existsSync(resolve(root, file))) throw new Error(`Final release file is missing: ${file}`);
...
if (!manifest.name.includes("AIR-DROW") || !manifest.start_url.includes("v740")) throw new Error("Install manifest is incomplete.");
...
if (!finalLayout.includes("brand-badge") || !finalLayout.includes("data-theme=\"light\"")) throw new Error("Final logo or light-mode contrast style is missing.");
