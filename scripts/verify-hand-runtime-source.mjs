...
for (const marker of ["vision_bundle.js", "vendor/mediapipe/wasm", "hand_landmarker.task?model=v620-hand-drawing"]) {
  if (!runtime.includes(marker)) throw new Error(`Hand runtime source configuration is missing ${marker}.`);
}
for (const marker of ["createLocalHandLandmarker", "local-url-gpu", "local-buffer-gpu", "local-url-cpu", "local-buffer-cpu", "FilesetResolver.forVisionTasks"]) {
