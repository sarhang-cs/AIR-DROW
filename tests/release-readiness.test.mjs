...
assert.equal(release.version, "8.0.0");
assert.equal(release.assetRevision, "800");
assert.match(source, /assets\/css\/production-ui\.css\?v=800/);
assert.doesNotMatch(source, /(?:manifest|assets\/css|toolbar-icons)\.\w+(?:\?v=)(?:770|790)/);
...
assert.match(worker, /assets\/css\/production-ui\.css\?v=800/);
assert.match(manifest.start_url, /build=v800/);
...
console.log("Release metadata, cache identity and safe gesture documentation QA passed.");
